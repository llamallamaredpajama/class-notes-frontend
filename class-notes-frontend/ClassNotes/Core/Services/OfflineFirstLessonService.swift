...
/// Offline-first implementation of LessonServiceProtocol following cursor rules
@MainActor
class OfflineFirstLessonService: LessonServiceProtocol, ObservableObject {
    ...
    private let provider: GRPCClientProvider
    private let modelContext: ModelContext
    private let logger = Logger(subsystem: "com.classnotes", category: "offline-lesson")
    ...
    init(modelContext: ModelContext) {
        ...
        self.provider = GRPCClientProvider.shared
            ...
            if networkMonitor.isConnected {
                Task {
                    do {
                        ...
                        logger.error("Failed to sync lesson: \(error)")
                    }
                }
            }
            return cachedLesson
                ...
                do {
                    ...
                    case .create:
                        if let payload = operation.payload,
                            let codableLesson = try? JSONDecoder().decode(
                                CodableLesson.self, from: payload)
                        {
                    ...
                    if operation.retryCount > 3 {
                        try cacheManager.removePendingOperation(operation)
                        OSLog.networking.error(
                            "Removed pending operation after 3 retries: \(operation.id)")
                    }
                }
            }
