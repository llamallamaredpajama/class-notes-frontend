...
final class GoogleSignInService: NSObject, ObservableObject, GoogleSignInServiceProtocol {
    ...
    func signIn() async throws -> GoogleUser {
        ...
        guard let presentingViewController = getRootViewController() else {
            throw AuthenticationError.unknown(NSError(
                domain: "GoogleSignIn",
                code: -1,
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            GIDSignIn.sharedInstance.signIn(withPresenting: presentingViewController) { [weak self] result, error in
                ...
                guard let result = result else {
                    let error = AuthenticationError.unknown(NSError(
                        domain: "GoogleSignIn",
                        code: -2,
                        userInfo: [NSLocalizedDescriptionKey: "No result from Google Sign-In"]
                    ))
                    self?.error = error
                ...
                let googleUser = self?.convertToGoogleUser(result.user) ?? GoogleUser(
                    id: "",
                    email: "",
                    displayName: "",
                    profileImageURL: nil,
